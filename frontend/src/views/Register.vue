<template>
  <div class="auth-wrapper">
    ...
    <div class="auth-container fade-in-up">
      <div class="top-line line-emerald"></div>
      ...
      <form @submit.prevent="submitRegister" class="auth-form">
        <div class="input-group">
          ...
          <div class="input-wrapper input-wrapper-emerald">
      ...
      <div class="auth-footer">
        <p>
          ...
          <router-link to="/" class="toggle-link emerald-link">{{ currentText.toLogin }}</router-link>
        </p>
      </div>
    </div>
  </div>
...
const translations = {
  tr: {
    title: "Hesap Oluştur",
    desc: "Platforma katılmak için yeni bir kullanıcı kaydı oluşturun.",
  ...
  en: {
    ...
    desc: "Create a new user registration to join the platform.",
...
const submitRegister = async () => {
  ...
  if (!email.value || !password.value || !confirmPassword.value) { errorMessage.value = currentText.value.errorEmpty; return; }
  ...
  if (password.value !== confirmPassword.value) { errorMessage.value = currentText.value.errorMismatch; return; }
...
<style scoped>
/* Stiller Login.vue ile aynı mantıkta ancak Emerald (Yeşil) renk temalıdır */
...
.line-emerald { background: linear-gradient(90deg, #10b981, #059669); }
...
.input-wrapper-emerald input:focus { border-color: #34d399; box-shadow: 0 0 0 3px rgba(52, 211, 153, 0.1); }
...
.emerald-link:hover { color: #6ee7b7; text-decoration: underline; }
